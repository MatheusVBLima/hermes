...
export async function execute(
    ...
    try {
        ...
        // Handle Starboard
        await StarboardService.handleReactionAdd(reaction, user);
        ...
        try {
            ...
            logger.info(`[ReactionRole] Added role ${role.name} to ${user.tag} in ${guild.name}`);
        } catch (error) {
            logger.error(`[ReactionRole] Failed to add role ${role.name} to ${user.tag}:`, error);
        }
    } catch (error) {
        logger.error('[Reaction] Error handling reaction add:', error);
    }
}
